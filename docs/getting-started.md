...
It now includes a **minimal Rust workspace bootstrap** with `shell-primitives` and early `shell-crypto` / `shell-state` / `shell-execution` / `shell-consensus` interface crates.
